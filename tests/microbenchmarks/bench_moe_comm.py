...
def _run_benchmark_worker_under_current_mpi(args: argparse.Namespace) -> None:
    ...
    for backend_name in backends:
        ...
        for local_num_tokens in local_batch_sizes:
            ...
            message_size = local_num_tokens * top_k * hidden_size * 2
            dispatch_bw_gbps = message_size / dispatch_stats["avg"] / 1000
            combine_bw_gbps = message_size / combine_stats["avg"] / 1000

            benchmark_results.append([local_num_tokens, top_k, hidden_size, message_size, dispatch_stats["avg"], dispatch_bw_gbps, combine_stats["avg"], combine_bw_gbps])
                    ...
                    json.dumps(
                        {
                            ...
                            "message_size(MB)": message_size / 1024 / 1024,
                            "dispatch_us": dispatch_stats,
                            "dispatch_bw(GB/s)": dispatch_bw_gbps,
                            "combine_us": combine_stats,
                            "combine_bw(GB/s)": combine_bw_gbps,
                        },
                        indent=2,
                    ),
                    flush=True,
        ...
        if rank == 0:
            ...
            for result in benchmark_results:
                print(f"local_num_tokens: {result[0]}, top_k: {result[1]}, hidden_size: {result[2]}, message_size: {result[3]} bytes, dispatch_us: {result[4]}, dispatch_bw: {result[5]} GB/s, combine_us: {result[6]}, combine_bw: {result[7]} GB/s")
            print("--------------------------------")
